...
from numpy import zeros, zeros_like, array, max, argmax, dot, unravel_index, outer, sum, sqrt, savez, load, prod
...
class MaxPooling:
	...
	def forward(self, input_data):
		...
		return array([
			[
				[
					max(input_data[n, i:i+self.pool_size, j:j+self.pool_size])
	...
	def backward(self, dloss_dout, learning_rate):
		dloss_dinput = zeros_like(self.input_data)
		for n in range(dloss_dinput.shape[0]):
			for i in range(0, dloss_dinput.shape[1], self.pool_size):
				for j in range(0, dloss_dinput.shape[2], self.pool_size):
					...
					max_index = argmax(window)
					max_i, max_j = unravel_index(max_index, window.shape)
...
class FullyConnected:
	def __init__(self, input_size, output_size, activation, rng):
		...
		self.biases = zeros(output_size)
	def forward(self, input_data):
		self.input_data = input_data
		self.score = self.weights @ input_data.flatten() + self.biases
		return self.activation(self.score)
	def backward(self, dloss_dout, learning_rate):
		dloss_dscore = dot(self.activation.derivative(self.score), dloss_dout)
		dloss_dweights = outer(dloss_dscore, self.input_data.flatten())
		dloss_dbiases = dloss_dscore
		dloss_dinput = dot(self.weights.T, dloss_dscore).reshape(self.input_data.shape)
